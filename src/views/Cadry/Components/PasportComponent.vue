  ...
  <div class="grid">
    ...
    <div class="col-12">
      ...
      >
        ...
        <div class="grid pt-2">
          <div class="col-12">
            <h6 class="mb-1 pl-2 text-yellow-500 font-normal">
              Pasport fayli sifatli va rangli ko'rinishda nusxasi yuklansin.
            </h6>
            <h6 class="mt-0 mb-2 pl-2 text-yellow-500 font-normal">
              2 , 3 sahiflari va yashash manzili sahifasi
            </h6>
...
export default {
  ...
  methods: {
    ...
    addandEdit() {
      ...
      if (!this.file_Error) {
        let sizeOfFile = this.file_path?.size / (1024 * 1024);
        if(sizeOfFile<=5){
          this.error_msg = null;
            let form = new FormData();
            form.append("file_path", this.file_path);
            this.controlDialog(false);
            PasportService.create_cadryPasport({
    ...
    uploadFile(event) {
      ...
      if (extention == "pdf" || extention == "PDF") {
        (this.error_msg = null), (this.file_path = files[0]);
        let size = files[0].size/(1024*1024);
        this.file_Name = `${files[0].name}   (${size}mb)`;
